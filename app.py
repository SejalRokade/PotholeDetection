    ...
    for file in uploaded_files:
        ...
        if file_type in ["jpg", "jpeg", "png"]:
            ...
            while True:
                ...
                results = model.track(frame, persist=True, tracker="bytetrack.yaml", conf=st.session_state.confidence)
                ...
                if boxes is not None:
                    if boxes.id is not None:
                        ...
                        for c_id in boxes.cls.cpu().numpy().astype(int):
                             if len(results[0].names) == 1:
                                 mapped_idx = hash(str(uuid.uuid4())) % len(class_names)
                                 class_counts[class_names[mapped_idx]] += 1
                             else:
